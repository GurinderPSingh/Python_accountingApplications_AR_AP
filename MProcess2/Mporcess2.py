...
def main():
    if len(sys.argv) != 3:
        print("Usage: python Mprocess2.py <file_path> <sheet_name>")
        return
    ...
    if not os.path.isdir(destination_folder):
        print(f"Error: The folder '{destination_folder}' does not exist.")
        return
    ...
    try:
        ...
        # Copy rows between "Name" and "ITS380"
        rows_to_copy = copy_rows(sheet)

        # If no rows to copy were found, print a message and exit
        if not rows_to_copy:
            print("No rows found between 'Name' and 'ITS380'.")
            return

        # Create or get "Page 2" sheet
        page_2_sheet = create_page_2_sheet(wb)

        # Paste rows to "Page 2" sheet
        paste_rows_to_page_2(page_2_sheet, rows_to_copy)

        # Find the last filled cell in column A and delete the corresponding row
        ...
        # Highlight rows and process other columns
        ...
        # Save the modified workbook to the specified destination
